...
public class StrongCareMultiple {
	...
	public static void main(String[] args) {
        ...
        for (int i = 0; i < locations.length; i++) {
            ...
            extent.createTest("test " + (i + 2))
                    .log(Status.INFO, "Entered Location");

            WebDriverWait wait1 = new WebDriverWait(driver, Duration.ofSeconds(30));
            WebElement clickElement = wait1.until(ExpectedConditions.elementToBeClickable(By.xpath("//p[@class='drug-search-result']")));
            clickElement.click();

            extent.createTest("test " + (i + 3))
                    .log(Status.INFO, "Selected location");

            driver.findElement(By.xpath("//input[@placeholder='Username/email']")).sendKeys(usernames[i]);
            extent.createTest("test " + (i + 4))
                    .log(Status.INFO, "Entered Username");

            driver.findElement(By.xpath("//input[@placeholder='Password']")).sendKeys(passwords[i]);
            extent.createTest("test " + (i + 5))
                    .log(Status.INFO, "Entered Password");

            WebDriverWait wait11 = new WebDriverWait(driver, Duration.ofSeconds(30));
            driver.findElement(By.xpath("//p[@class='blue-button']")).click();
            extent.createTest("test " + (i + 6))
                    .log(Status.INFO, "Clicked Login Button");
            ...
            dropdown.click();
            extent.createTest("test " + (i + 7))
                    .log(Status.INFO, "Clicked Dropdown");

            driver.findElement(By.xpath("//p[@class='blue-button']")).click();
            extent.createTest("test " + (i + 8))
                    .log(Status.INFO, "Selected Internal facility");
